...
public class StatisticsServiceImplTest {
	...
	public void shouldFindDataPointListByAccountName() {
		...
		when(repository.findByPointIdAccount("test")).thenReturn(list);
	...
	public void shouldSaveDataPoint() {
		...
		salary.setCurrency(Currency.USD.name());
		salary.setPeriod(TimePeriod.MONTH.name());

		Item grocery = new Item();
		grocery.setTitle("Grocery");
		grocery.setAmount(new BigDecimal(500));
		grocery.setCurrency(Currency.RUB.name());
		grocery.setPeriod(TimePeriod.DAY.name());

		Item vacation = new Item();
		vacation.setTitle("Vacation");
		vacation.setAmount(new BigDecimal(3400));
		vacation.setCurrency(Currency.EUR.name());
		vacation.setPeriod(TimePeriod.YEAR.name());

		Saving saving = new Saving();
		saving.setAmount(new BigDecimal(1000));
		saving.setCurrency(Currency.EUR.name());
		...
		final Map<Currency, BigDecimal> rates = ImmutableMap.of(Currency.EUR, new BigDecimal("0.8"), Currency.RUB,
		...
		when(ratesService.convert(any(Currency.class), any(Currency.class), any(BigDecimal.class)))
				.then(i -> i.getArgumentAt(2, BigDecimal.class).divide(rates.get(i.getArgumentAt(0, Currency.class)), 4,
		...
		assertEquals(dataPoint.getPointId().getAccount(), "test");
		assertEquals(dataPoint.getPointId().getDate(),
