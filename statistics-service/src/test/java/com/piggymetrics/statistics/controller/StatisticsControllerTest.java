...
public class StatisticsControllerTest {
	...
	public void shouldGetStatisticsByAccountName() throws Exception {
		...
		dataPoint.setPointId(new DataPointId("test", new Date()));

		when(statisticsService.findByAccountName(dataPoint.getPointId().getAccount()))
				.thenReturn(ImmutableList.of(dataPoint));

		mockMvc.perform(get("/test").principal(new UserPrincipal(dataPoint.getPointId().getAccount())))
				.andExpect(jsonPath("$[0].id.account").value(dataPoint.getPointId().getAccount()))
	...
	public void shouldGetCurrentAccountStatistics() throws Exception {
		...
		mockMvc.perform(get("/current").principal(new UserPrincipal(dataPoint.getPointId().getAccount())))
				.andExpect(jsonPath("$[0].id.account").value(dataPoint.getPointId().getAccount()))
				.andExpect(status().isOk());
	}
	...
	public void shouldSaveAccountStatistics() throws Exception {
		...
		saving.setCurrency(Currency.USD.name());
		...
		grocery.setCurrency(Currency.USD.name());
