...
function initAccount(account) {
	statsNotificationCalled(account);
	user = new User(account.name, account.lastSeen, account.saving.currency,
			account.note);
	savings = new Savings(account.saving.amount, account.saving.deposit,
			account.saving.capitalization, account.saving.interest);

	if (account.incomes) {
		for (i = 0; i < account.incomes.length; i++) {
			AddIncome(i + 1, account.incomes[i].title, account.incomes[i].icon,
					account.incomes[i].currency, account.incomes[i].period,
					account.incomes[i].amount);
		}
	}

	if (account.expenses) {
		for (j = 0; j < account.expenses.length; j++) {
			AddExpense(j + 1, account.expenses[j].title,
					account.expenses[j].icon, account.expenses[j].currency,
					account.expenses[j].period, account.expenses[j].amount);
		}
	}
}

function statsNotificationCalled(account) {
	// Calling Stats and Notification Service
	$.ajax({
...
function User(username, lastSeen, currency, note) {
	...
	this.login = username;
	this.lastSeen = (seen.getMonth() + 1) + "/" + seen.getDate() + "/"
			+ seen.getFullYear();
	;
...
function initGreetingPage() {
	...
	initStatisticPage();
	setTimeout(function() {
		initSavingsCircles(user.checkedPercent, 0.2, savings.freeMoney,
				savings.freeMoney, 700)
	}, 200);
	initSavingsSlider();
	$("#savings-slider").data({
		"checkedPercent" : user.checkedPercent
	});
...
function initSettingsPage() {
	...
	$("#overlay, .modal-content").show(); // Fix transition and visibility:
	// hidden Chrome issue
...
function greetingPageAgain() {
	...
	/*
	 * Initiate settings page on press plus or enter $(document).on("keyup",
	 * function (e) { if (e.which == 13) { setTimeout(initSettingsPage, 400); }
	 * });
	 */
	...
	$("#righttitle").append(
			'<span class="bluetext">last seen: </span>' + user.lastSeen);
	$("#lefttitle").append(
			escape(user.login) + '<span class="bluetext"> metrics</span>');
}
...
function addItems() {
	Object
			...
			.forEach(
					function(key) {
						var value = incomes[key].amount.replace(
								/(\d)(?=(\d\d\d)+([^\d]|$))/g,
								'$1 </span><span class="lightdigit20">');
						$("#incomeslider")
								.append(
										'<div onclick="itemClick(this)" class="incomeitem" id="income-'
	...
	Object
			...
			.forEach(
					function(key) {
						var value = expenses[key].amount.replace(
								/(\d)(?=(\d\d\d)+([^\d]|$))/g,
								'$1 </span><span class="lightdigit20">');
						$("#expenseslider")
								.append(
										'<div onclick="itemClick(this)" class="expenseitem" id="expense-'
...
// According to number of column items - show/hide up&down buttons and change
// position:absolute
...
function Up(transaction) {
	...
	if (Math.abs($("#" + transaction + "slider").position().top % itemWidth) > 0.01)
		return;
	else if ($("#" + transaction + "slider").position().top > -(length - 4)
			* itemWidth) {
		var pixels = $("#" + transaction + "slider").position().top
				+ (length - 5) * itemWidth;
...
// ADD ITEMS
function addNewItem() {
	var itemTitle = $(".modaltitle").val(), itemIcon = $(".initicons").data(
			"iconselected"), itemCurrency = $(".modalcurrency").val(), itemPeriod = $(
			".modalperiod").val(), itemValue = $(".modalvalue").autoNumeric(
			"get"), itemId = 0;
	if (checkModalFields(itemValue, itemTitle)) { // If inputs are proper
		// filled, add incomes or
		// expenses
		if ($(".initicons").data("incomes-expenses") == "incomes") {
			...
			AddIncome(++itemId, itemTitle, itemIcon, itemCurrency, itemPeriod,
					itemValue);
			addNewDiv("income", itemId, itemTitle, itemIcon, itemCurrency,
					itemPeriod, itemValue);
		} else {
			...
			AddExpense(++itemId, itemTitle, itemIcon, itemCurrency, itemPeriod,
					itemValue);
			addNewDiv("expense", itemId, itemTitle, itemIcon, itemCurrency,
					itemPeriod, itemValue);
		}
		turnOffModal();
	}
...
function addNewDiv(whichColumn, itemId, itemTitle, itemIcon, itemCurrency,
		itemPeriod, itemValue) {
	var value = itemValue.replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g,
			'$1 </span><span class="lightdigit20">');
	$("#" + whichColumn + "slider")
			.append(
					'<div onclick="itemClick(this)" class="'
							+ whichColumn
							+ 'item" id="'
							+ whichColumn
							+ '-'
...
function itemClick(item) {
	// Add delete-button
	$(".modal-delete").show();
	var itemDiv = $("#" + item.id), itemIcon = itemDiv.data("icon"), itemCurrency = itemDiv
			.data("currency"), itemPeriod = itemDiv.data("period"), itemValue = itemDiv
			.data("amount"), itemTitle = itemDiv.data("title"), itemId = itemDiv
			.data("id"), incomesExpenses = "expense", whichColumn = expenses;
	...
	$(".initicons").data({
		"iconselected" : itemIcon,
		"add-edit" : "edit",
		"incomes-expenses" : incomesExpenses + "s"
	});
	...
	saveOldItem = function() {
		if (checkModalFields($(".modalvalue").val(), $(".modaltitle").val())) { // If
			// inputs
			// are
			// proper
			// filled,
			...
			editOldDiv(incomesExpenses, itemId, $(".modaltitle").val(), $(
					".initicons").data("iconselected"), $(".modalcurrency")
					.val(), $(".modalperiod").val(), $(".modalvalue")
					.autoNumeric("get"));
			turnOffModal();
		}
		checkSlidersLength();
...
function editOldDiv(whichColumn, itemId, itemTitle, itemIcon, itemCurrency,
		itemPeriod, itemValue) {
	var value = itemValue.replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g,
			'$1 </span><span class="lightdigit20">');
	$("#" + whichColumn + "-" + itemId)
...
$(".modalvalue").bind("keyup keydown keypress select focus click",
		modalFontSize);
...
$("#rublesign").on(
		...
		function() {
			...
			changeCurrency();
			$("#savingsvalue")
					.autoNumeric('set', Math.round(savings.freeMoney));
			moveRuble();
			// Update savings slider
			$('#savings-slider').noUiSlider(
					{
						start : (incomesSumMonth - expensesSumMonth)
								* $("#savings-slider").data("checkedPercent"),
...
$("#logoclickplace")
		.click(
				function() {
					$(
							"#logo_settings, #overlay, .modal-content, #settingspage, #expenseslider, #bubble")
							.fadeOut(300);
					setTimeout(greetingPageAgain, 250);
				});

// All-PAGE SWIPE BETWEEN 3-4 PAGES
$("#swipefield, #savings, #savebutton, #settings_hat").bind(
...
function launchStatistic() {
	if ($("#incomeslider").children().length > 0
			&& $("#expenseslider").children().length > 0) {
...
function jsonDataSave() {
	if (global.savePermit) {
		...
		$.ajax({
			...
			data : check,
			success : function() {
				$("#leftborder, #rightborder, #centerborder").addClass(
						"saveaction");
				setTimeout(function() {
					$("#leftborder, #rightborder, #centerborder").removeClass(
							"saveaction");
				}, 400);
			},
			error : function() {
